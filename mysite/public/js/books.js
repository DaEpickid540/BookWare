...
export async function lookupISBN(isbn) {
  ...
  const ol = await safeFetch(`${OL_SEARCH}?q=isbn:${encodeURIComponent(clean)}&limit=1`, 'OL ISBN');
  if (ol?.docs?.length) { const p = parseOL(ol.docs[0]); if (p) return p; }
  const g = await safeFetch(`${GBOOKS}?q=isbn:${encodeURIComponent(clean)}&maxResults=1`, 'Google ISBN');
...
export async function searchBooks(query, max = 10) {
  ...
  if (ol?.docs?.length) return ol.docs.map(parseOL).filter(Boolean);
  const g = await safeFetch(
    `${GBOOKS}?q=${encodeURIComponent(q)}&maxResults=${Math.min(max, 40)}&printType=books`,
    'Google search',
  );
  if (g?.items?.length) return g.items.map(parseGoogle).filter(Boolean);
  return [];
}
